...
void ifft(float in[][2], float out[], const int N){
  ...
  int m = 0;
  for(int n=0; n<N; n++){/* computes top level by reordering input */
    int n1 = reverse(m++);
    int n2 = reverse(m++);
    if(n < 32){
  ...
  for(int L=D-2; L>=0; L--){/* compute lower levels in terms of one above */
    int j = 0;
    for(int n=0; n<64; n++){
      int A = D+1;
      int J = 1<<A;
  ...
  for(int n=0; n<N; n++){/* compute output from bottom level */
    float* T0 = T[0][0];
    float* T1 = T[0][1];
    float angle = PI2*n/N;
    float C[2] = {cos(angle), sin(angle)};
