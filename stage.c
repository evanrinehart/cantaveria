...
stages are 320x240 regions
zone is an array of stages
tiles are 16x16 there are 20x15 in a stage
tiles can be not there (see through)
tiles can be background (walk through)
tiles can be slanted 45 or atan(1/2) degrees
some tiles can have the water flag.

stages have four sides. each of the four sides
can have a lock option which causes the camera
to not look past that side. this is easy with 320x240
rez, but with widescreen, this situation causes
black to be shown past the boundary (even if there
is another stage there). when you move from one
stage to another the camera 'snaps' to the other
stage if there was a 'no show' option on that edge.
if there is no wall and there is no stage, the
'no stage' acts as a wall

zones are disconnected in space, but are
connected by exits. a stage can have an exit
on one side which leads to a stage in another zone

coordinate systems will be confusing:
stages inside a zone are on stage indexes relative to zone top left
zones are absolutely positioned using stage indexes
entities are absolutely positioned using 1024th pixel coordinate
camera is absolutely positioned similar to entities
the absolute zero of 1024th pixel / zone pos / stage index is
in the middle of the game world, because we are using signed numbers

screen pos of entity = (entpos - camera) / 1024
screen pos of stage = (stageindex + zoneindex)*(320,240) - camera/1024


entities exist on a 1024th pixel system, centered on zone

1. stages
2. fg bg slant none water tiles
3. stage edge camera snap
3. edge stage edge 'no wall wall'
4. fg decorations
5. zone exits
6. stage collision
...
print a zone to stdout
print a stage to stdout
*/
